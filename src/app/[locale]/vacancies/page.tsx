...
type Job = {
  ...
  subtitle: string;
  body: string;
  href: string; // e.g. "/vacancies/my-slug" or "https://external-form…"
};
...
export default function Vacancies() {
  ...
  // helper: add locale prefix unless it's an absolute URL
  const withLocale = (href: string) =>
    /^https?:\/\//i.test(href) ? href : `/${locale}${href.startsWith("/") ? "" : "/"}${href}`;
      ...
      <section className="bg-[#EDF3FF]">
        <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 md:px-8 py-12 md:py-16">
          <ul className="divide-y divide-slate-200/80">
            ...
            {jobs.map((job, i) => {
              const internal = !/^https?:\/\//i.test(job.href || "");
              const href = job?.href ? withLocale(job.href) : `/${locale}/vacancies/${job.slug ?? ""}`;
              return (
                <li key={i}>
                  <NextLink
                    href={href}
                  ...
                  >
                    <div>
                      <h3 className="text-base sm:text-lg font-semibold text-slate-900">
                        {job.title || job.slug || "Untitled vacancy"}
                      </h3>
