...
export async function GET(req: Request) {
  ...
  // example ordering: by slug (you can customize)
...
export async function POST(req: Request) {
  ...
  if (ct.includes("application/json")) {
    ...
    if (String(b.action || "") === "delete") {
      ...
      return handleUpsert({
        ...
        subtitle: String(b.subtitle || ""), // <-- NEW
        body: String(b.body || ""),
        href: String(b.href || ""),
      });
    }
  ...
  if (String(fd.get("action") || "") === "delete") {
    return handleDelete({
      ...
      all:
        String(fd.get("all") || "") === "1" ||
        String(fd.get("all") || "").toLowerCase() === "true",
    });
  } else {
    return handleUpsert({
      ...
      subtitle: String(fd.get("subtitle") || ""), // <-- NEW
...
async function handleUpsert(params: {
  ...
  // merge existing translation (preserve other langs)
  ...
  return NextResponse.json(
    { ok: true, id: item.id, slug: item.slug },
